...
class LatexTableMaker(LatexTableFunctions):
  ...
  def __init__(self, in_file, extraction_type='feature_selection'):
    ...
    self.feature_re = r'_c[01]d[01]d[01]e[01]_'
    self.data_size_re = r'_f-[1-3]x[0-9]+x[0-9]+_'
  ...
  def get_train_instance_infos(self):
    ...
    for ti in train_instances:
      ...
      # feature
      feature_sel = [int(i) for i in re.sub(r'[cde_]', '', re.findall(self.feature_re, ti)[0])]

      # datasize
      data_size = re.split('x', re.sub(r'[f\-_]', '', re.findall(self.data_size_re, ti)[0]))
