...
class HybJimG(nn.Module, AdvBasics):
  """
  hybrid jim network
  """
  ...
  def __init__(self, n_classes, data_size, n_latent=100, n_feature_maps_l0=48):
    ...
    from adversarial_nets import Adv_G_Jim
    ...
    # generator net
    self.generator = Adv_G_Jim(self.n_classes, self.data_size, n_latent=self.n_latent, n_feature_maps_l0=48, is_last_activation_sigmoid=True)

    # init weights
    self.apply(self.weights_init)

    # share weight
    self.conv_layer0.weight = self.generator.deconv_layer1.weight
    self.conv_layer1.weight = self.generator.deconv_layer0.weight
  ...
  def forward(self, x):
    ...
    # create fakes
    fakes = self.generator(torch.randn(x.shape[0], self.n_latent))
...
if __name__ == '__main__':
  ...
  # create model
  #model = HybJim(n_classes=5, data_size=x.shape[1:])
  model = HybJimG(n_classes=5, data_size=x.shape[1:])

  # test model
  o_x, o_y = model(x)

  # print some infos
  print("\nx: ", x.shape), print("model: ", model), print("o: ", o_x), print("o: ", o_y.shape)