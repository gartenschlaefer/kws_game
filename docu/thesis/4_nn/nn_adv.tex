...
Note that the adversarial pre-training methods perform merely on the \texttt{adv-d-jim}, \texttt{adv-g-jim}, and \texttt{conv-jim} models, which are described in \rsec{nn_arch}.
...
In \req{nn_theory_gan} the game is notated as min-max game, from which the loss of D, denoted as $l_D$, can be described for one specific training example $i$ as follows:
\begin{equation}
