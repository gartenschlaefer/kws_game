...
\begin{enumerate}
	...
	\item Wavenets
\end{enumerate}
where CNNs were used for the classification of MFCC features and are therefore the main architecture type within this thesis.
...
Deployed in an online system, the Wavenet architecture has no need to extract MFCC features.
...
The second lowest footprint is given by the \texttt{conv-jim} model and the \texttt{conv-trad} model requires the highest amount of operations.
...
This implies that the kernel size of the filters must be equal for each layer, the amount of feature maps does not necessarily have to be the same.
...
However it is preferred to use a normalization technique, such as frame-based normalization for the G network, otherwise the transferring of weights might be disadvantageous for the classifier network.
...
An classical implementation of a Wavenet can be found in \cite{Herrmann2018} and motivated the following model structure.
...
\FloatBarrier
\noindent
It is important to mention that the convolutional layers in the residual blocks should not use bias terms, because they led to bad training results in the conducted experiments of this thesis.
...
The whole Wavenet architecture adaption with an extension of a class prediction is shown in \rfig{nn_arch_wavenet_all}.
\begin{figure}[!ht]
  \centering
...
Further the residual blocks do not decrease the dimensions of their output tensors.