...
For all nodes in a current layer the equation \req{nn_theory_fc_single} can be summarized in a weight matrix $W \in \R^{m \times n}$, where $m$ is the total amount of nodes in the current layer and $n$ is the total number of nodes from the previous layer.
