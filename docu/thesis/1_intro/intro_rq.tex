...
The microphone will therefore not only capture the voice of a speaker, but also a fair amount of game sounds. 
...
Without doubt players will try out words, which are not in the vocabulary (denoted as \emph{unknown} key words) and observe the response of the video game.
...
At the same time the neural network should not classify key words, as unknown key words, to create a satisfying game play.
...
Further it is important to recognize the amount of time lag required to process speech commands to actual actions within the game.
The player should for one get immediate and accurate feedback from the game and on the other hand be challenged while playing, so that the game play does not get frustrating or boring.
