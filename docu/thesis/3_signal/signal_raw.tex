...
The audio files provided in the speech command dataset \cite{Warden2018} used for the experiments in \rsec{exp}, are recorded with a sampling rate of \SI{16}{\kilo\hertz}, which is totally enough for human speech signals.
...
For evaluation and visualization purpose the author of this thesis recorded his own examples from the speech commands \{\enquote{left}, \enquote{right}, \enquote{up}, \enquote{down}, \enquote{go}\}.
...
The example are shown in \rfig{raw_audio_my} in raw audio format.
\begin{figure}[!ht]
  \centering
...
It is easy to see in \rfig{raw_audio_my} where the words are starting and ending, but usually not all recordings are as clean as those.
