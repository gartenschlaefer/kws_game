...
\FloatBarrier
\noindent
The frame-based normalization is an interesting technique in order to improve the visualization of MFCC features.
...
Note that the computation of the deltas at the edges $m=0$ and $m=M - 1$ is not possible and that the same value is obtained from the neighbor at this specific positions.
