...
The energy consumption is evaluated on the amount of operations needed to process a \SI{500}{\milli\second} time signal sampled with $f_s = \SI{16}{\kilo\hertz}$ to MFCC features.
...
One complex multiplication equals to 4 real multiplication and 2 additions, so one complex multiplication is equal to 6 operations.
...
This of course are a lot of operations, luckily a more sophisticated Fourier transform is done by the Fast Fourier Transform (FFT) method, which gives a linear complexity of
\begin{equation}
