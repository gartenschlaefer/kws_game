...
Two experiments perform on the MFCC feature selection with:
\begin{enumerate}
    \item Impact on the amount of cepstral coefficients and
...
All experiments fixed the total number of filter bands of the MFCCs to 32 bands.
...
The experiments are performed once with and once without frame-based normalization to observe its impact on the classification accuracy, as well as on the noise and shift invariance.
...
However it usually does not achieve that good shift invariance compared to the other models, and often holes appear in the classification with the shift invariance test.
...
The usage of 32 MFCC coefficients does not much improve the invariance against shift or noise, but often worsens the results.
...
The frame-based normalization was applied with results of the experiments listed in \rtab{exp_fs_mfcc_l12}.
