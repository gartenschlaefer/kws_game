...
The folder named as \texttt{\_background\_noise\_} contains six different background noise files, such as \texttt{white\_noise.wav} or \texttt{doing\_the\_dishes.wav}, with a duration of more than one minute each and altogehter summing up to a duration of about \SI{400}{s}.
...
The feature extraction of the dataset with specified parameters are stored to separate files before using them for training.
...
The neural network architectures are trained with supervised learning, that means a class label $y_i \in \{0, 1, \dots, L\}$ correspondence to each data example $x_i \in \R^{\mathcal{X}}$ must exist, where $L$ is the total number of classes and $\mathcal{X}$ is the dimension of the data example.
