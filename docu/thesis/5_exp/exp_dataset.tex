...
Some examples of the speech command dataset in raw audio format are shown in \rfig{exp_dataset_wav_grid_speech_commands_v2} and give a small glimpse on the quality of the recordings.
\begin{figure}[!ht]
  \centering
...
It was already shown how it is possible to extract MFCC features in \rsec{signal_mfcc}, however it is important that each individual $x_i$ for all $i$ has the same dimension.
