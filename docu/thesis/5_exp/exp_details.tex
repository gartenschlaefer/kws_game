...
The deployed video game in \rsec{game} uses only 7 of those L12 labels, the rest is handled in the same way as the \enquote{\_mixed} label.
...
Note that the action set of the video game is displayed informatively and not used in the experiments, though it could have improved the classification performance in the deployed video game if only 7 labels instead of the L12 were used.
Nevertheless the deployed video game worked well with the model trained on the L12 labels and very few confusions are happening to the unused key words.
