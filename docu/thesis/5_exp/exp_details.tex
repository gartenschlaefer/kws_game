...
The training details provide the used sets of hyperparameters for training the neural networks on the speech commands dataset.
...
All matrix-vector computations were done with the well known \texttt{Numpy} package or with \texttt{Pytorch}.
...
Still the video game operates with the models trained on the L12 labels and thus might increase the change in confusing keywords.
...
This might result into a decrease of the accuracy performance if the set of parameters is used at the last epoch and no early stopping mechanism was applied.
...
The hyperparameters, shown in \rtab{exp_details_params_pre_train}, are the same as for usual training but the Discriminator and Generator network can have each different values.
...
Conversely, the shift invariance test analyzes whether there are consecutive correct classifications within a certain region on the shift index.
Holes in this region are not a good indicator for the trained model.
If one example could not be classified at all, it does not necessarily imply that the trained model performs poorly but that this specific example is not recognized with this specific model.
...
This is because a significant amount of noise may be added from the use of poor microphones or recording set-ups and therefore might disturb the classification accuracy.
